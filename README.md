...
```bash
# Claude Code
ln -s "$(pwd)" ~/.claude/skills/design-fitness-gym-salon
# Codex CLI
ln -s "$(pwd)" ~/.codex/skills/design-fitness-gym-salon
```

Restart the agent; it is matched automatically by the skill's `description` (skill name: `design-fitness-gym-salon`). / エージェント再起動後、`description` に基づき自動マッチします。
